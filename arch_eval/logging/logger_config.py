...
def setup_logging(
    level: str = "INFO", log_file: Optional[str] = None, fmt: Optional[str] = None, force: bool = False
) -> logging.Logger:
    ...
    if force:
        ...
        has_file = log_file and any(
            isinstance(h, RotatingFileHandler) and h.baseFilename == os.path.abspath(log_file) for h in root.handlers
        )
