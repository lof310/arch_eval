...
def _train_single_process(args):
    ...
    try:
        trainer_config = TrainingConfig(
            ...
            # Training-specific fields from BenchmarkConfig
            mixed_precision=config.mixed_precision,
            mixed_precision_dtype=config.mixed_precision_dtype,
            gradient_clip=config.gradient_clip,
            checkpoint_dir=config.checkpoint_dir,
            optimizers=list(config.optimizers) if config.optimizers else [],
            schedulers=list(config.schedulers) if config.schedulers else [],
            callbacks=list(config.callbacks) if config.callbacks else [],
            loss_function=config.loss_function,
            model_output_transform=config.model_output_transform,
            ...
            log_all_metrics=config.log_all_metrics,
            input_shape=config.input_shape,
            loss_fn_extractor=config.loss_fn_extractor,
            loss_mode=config.loss_mode,
            distributed_backend=config.distributed_backend,
            distributed_world_size=config.distributed_world_size,
            distributed_rank=config.distributed_rank,
            distributed_master_addr=config.distributed_master_addr,
            distributed_master_port=config.distributed_master_port,
            gradient_checkpointing=config.gradient_checkpointing,
            gradient_checkpointing_modules=config.gradient_checkpointing_modules,
            profiler=config.profiler,
            gc_collect_interval=config.gc_collect_interval,
            log_confusion_matrix=config.log_confusion_matrix,
            confusion_matrix_labels=config.confusion_matrix_labels,
            debug=config.debug,
            compile_model=config.compile_model,
            compile_kwargs=dict(config.compile_kwargs) if config.compile_kwargs else {},
    ...
    def _train_single(self, model_info):
        ...
        try:
            trainer_config = TrainingConfig(
                ...
                mixed_precision_dtype=self.config.mixed_precision_dtype,
                gradient_clip=self.config.gradient_clip,
                checkpoint_dir=self.config.checkpoint_dir,
                optimizers=list(self.config.optimizers) if self.config.optimizers else [],
                schedulers=list(self.config.schedulers) if self.config.schedulers else [],
                callbacks=list(self.config.callbacks) if self.config.callbacks else [],
                loss_function=self.config.loss_function,
                model_output_transform=self.config.model_output_transform,
                ...
                log_all_metrics=self.config.log_all_metrics,
                input_shape=self.config.input_shape,
                loss_fn_extractor=self.config.loss_fn_extractor,
                loss_mode=self.config.loss_mode,
                distributed_backend=self.config.distributed_backend,
                distributed_world_size=self.config.distributed_world_size,
                distributed_rank=self.config.distributed_rank,
                distributed_master_addr=self.config.distributed_master_addr,
                distributed_master_port=self.config.distributed_master_port,
                gradient_checkpointing=self.config.gradient_checkpointing,
                gradient_checkpointing_modules=self.config.gradient_checkpointing_modules,
                profiler=self.config.profiler,
                gc_collect_interval=self.config.gc_collect_interval,
                log_confusion_matrix=self.config.log_confusion_matrix,
                confusion_matrix_labels=self.config.confusion_matrix_labels,
                debug=self.config.debug,
                compile_model=self.config.compile_model,
                compile_kwargs=dict(self.config.compile_kwargs) if self.config.compile_kwargs else {},
